...
ALTER PROCEDURE [dbo].[pNLI_InsertarNuevoExamenVerificador]  
 @Lista varchar(max),  
...
SET NOCOUNT ON      
 ...
 Declare @fechaColeccion varchar(20)  
 Declare @horaColeccion varchar(20)  
 ...
 DECLARE @idOrdenExAnt varchar (50)  
  ...
  INSERT INTO OrdenExamen (idOrdenExamen,idOrden,idEnfermedad,idTipoMuestra,idExamen,estatusE,estatus,estado,  
      ...
      SELECT             @newIdOrdenExamen, @idOrden, @idEnfermedad, @idTipoMuestra, @idExamen, 7,  1,  1,   
         fechaRegistro, idUsuarioRegistro, GETDATE(),@idUsuario FROM OrdenExamen WHERE idOrden = @idOrden and estatus = 11  
  ...
  INSERT INTO OrdenMuestra (idOrdenMuestra,idOrden,idTipoMuestra,idProyecto,idMuestraCod,fechaColeccion,horaColeccion,   
         ...
         SELECT    @newIdOrdenMuestra,@idOrden,@idTipoMuestra,idProyecto,idMuestraCod,fechaColeccion,horaColeccion,numero,  
         seriado,1,1,fechaRegistro,idUsuarioRegistro,GETDATE(),@idUsuario from OrdenMuestra WHERE idOrden = @idOrden and   
         fechaColeccion = (select MIN(fechaColeccion)from OrdenMuestra where idOrden = @idOrden)   
  ...
  INSERT INTO OrdenMaterial (idOrdenMaterial, idOrden, idOrdenMuestra, idOrdenExamen, idMaterial, idLaboratorio, Tipo, fechaEnvio, horaEnvio, cantidad,   
      ...
      SELECT           @newIdOrdenMaterial,@idOrden,@newIdOrdenMuestra,@newIdOrdenExamen,@idMaterial,@idLaboratorio,tipo,@fechaColeccion,@horaColeccion,  
  ...
  INSERT INTO OrdenMuestraRecepcion (idOrdenMuestraRecepcion, idOrden, idOrdenMaterial, fechaRecepcion, horaRecepcion, idLaboratorioOrigen, idLaboratorioDestino, fechaEnvio,   
      ...
      SELECT        @newIdOrdenMuestraRecepcion,@idOrden,@newIdOrdenMaterial,@fechaColeccion,@horaColeccion,@idEstablecimiento,@idLaboratorio,@fechaColeccion,  
             @horaColeccion,2,1,1,GETDATE(),@idUsuario,idOrdenMuestraRecepcion FROM OrdenMuestraRecepcion where idOrden = @idOrden   
 ...
 END  
  ...
  INSERT INTO OrdenExamen (idOrdenExamen,idOrden,idEnfermedad,idTipoMuestra,idExamen,estatusE,estatus,estado,  
      ...
      SELECT             @newIdOrdenExamen, @idOrden, @idEnfermedad, @idTipoMuestra, @idExamen, 7,  1,  1,   
         fechaRegistro, idUsuarioRegistro, GETDATE(),@idUsuario FROM OrdenExamen WHERE idOrden = @idOrden  
         and fechaIngreso = (select MIN(fechaIngreso)from OrdenExamen where idOrden = @idOrden)   
  ...
  INSERT INTO OrdenMuestraRecepcion (idOrdenMuestraRecepcion, idOrden, idOrdenMaterial,conformeR, fechaRecepcion, horaRecepcion, idLaboratorioOrigen, idLaboratorioDestino, fechaEnvio,   
      ...
      SELECT        @newIdOrdenMuestraRecepcion,@idOrden,@newIdOrdenMaterial,1,@fechaColeccion,@horaColeccion,@idEstablecimiento,@idLaboratorio,@fechaColeccion,  
             @horaColeccion,@fechaColeccion,@horaColeccion,@idUsuario,1,getdate(),@idUsuario,3,6,3,1,GETDATE(),@idUsuario,idOrdenMuestraRecepcion FROM OrdenMuestraRecepcion where idOrden = @idOrden and estatusP=6  
             and fechaRecepcion = (select MIN(fechaRecepcion) from OrdenMuestraRecepcion where idOrden = @idOrden)  
